...
from ...utils import valve_curve, memo, print_time_delta
from ...utils import calc_parabola_vertex, configure_logging
...
def MOCSimulator(
    ...
    logger.info("Running Pure Python MOCSimulator...")
    ...
    for ts in range(1,tn):
        ...
        if ts == 2:
            for _,pipe in tm.pipes():
                ...
                if abs(diff1)> 5e-1:
                    logger.warning('Initial condition discrepancy of pressure (%.4f m) on the %s node' %(diff1,pipe.start_node.name))
                if abs(diff2)> 5e-1:
                    logger.warning('Initial condition discrepancy of pressure (%.4f m) on the %s node'%(diff2,pipe.end_node.name))
        if ts == 3:
